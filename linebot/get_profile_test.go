...
func TestGetGroupProfile(t *testing.T) {
	type want struct {
		URLPath     string
		RequestBody []byte
	...
	var testCases = []struct {
	...
	}{
		{
			...
			Want: want{
				URLPath:     fmt.Sprintf(APIEndpointGetGroupProfile, "cxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"),
				RequestBody: []byte(""),
				Response: &GroupSummaryResponse{
					GroupID:    "Ca56f94637c...",
	...
	for i, tc := range testCases {
		currentTestIdx = i
		t.Run(strconv.Itoa(i)+"/"+tc.Label, func(t *testing.T) {
			res, err := client.GetGroupProfile(tc.GroupID).Do()
			if tc.Want.Error != nil {
				if !reflect.DeepEqual(err, tc.Want.Error) {
					t.Errorf("Error %v; want %v", err, tc.Want.Error)
				}
